...
# Assuming 'test' is on Python's path or the app is run from 'test' directory
# so 'config' and 'utils' are top-level accessible from 'test' perspective.
...
from utils.core_data_processing import (
    ...
    # are now more likely used within component modules, but can be here if needed.
)
from utils.ai_analytics_engine import (
    apply_ai_models,
    # SupplyForecastingModel might be used in supply_chain_tab.py directly
)

# Corrected relative imports assuming 'clinic_components' is a sub-package of 'pages'
# This requires 'test/pages/clinic_components/__init__.py' to exist (can be empty).
