...
def get_clinic_dashboard_data_enriched():
    ...
    iot_df = load_iot_clinic_environment_data(file_path=app_config.IOT_CLINIC_ENVIRONMENT_CSV) # Load IoT data

    if health_df_raw.empty:
        logger.error("Clinic Dashboard: Failed to load health records from core processing.")
        # Return tuple with empty health_df but potentially valid iot_df
    ...
    elif health_df_ai_enriched.empty: # Both failed
...
# --- Main Page Rendering ---
# Handle cases where data might be partially or fully missing
...
if not health_data_available:
    ...
    # We can still proceed to render the title and sidebar, and IoT parts if available
    # st.stop() # Avoid stopping if IoT data might still be useful
...
all_dates_clinic = []
if health_data_available and 'encounter_date' in health_df_clinic_main:
    all_dates_clinic.extend(pd.to_datetime(health_df_clinic_main['encounter_date'], errors='coerce').dropna())
if iot_data_available and 'timestamp' in iot_df_clinic_main:
...
if health_data_available:
    health_df_clinic_main['encounter_date_obj'] = pd.to_datetime(health_df_clinic_main['encounter_date']).dt.date
    filtered_health_df_clinic = health_df_clinic_main[
...
if iot_data_available:
    iot_df_clinic_main['timestamp_date_obj'] = pd.to_datetime(iot_df_clinic_main['timestamp']).dt.date
    filtered_iot_df_clinic = iot_df_clinic_main[
    ...
    if filtered_iot_df_clinic.empty and iot_data_available: # If main IoT has data, but not for period
...
elif not iot_data_available: # If main IoT was empty from start
...
# Initialize clinic_service_kpis even if health data is empty, to avoid errors downstream
...
with kpi_cols_main_clinic[2]:
    ...
    render_kpi_card("Pending Critical Tests", str(pending_crit_val), "⏳",
                    status="High" if pending_crit_val > 10 else ("Moderate" if pending_crit_val > 0 else "Low"), # Example thresholds
...
# Test summary details for individual test KPIs
test_details_for_kpis = clinic_service_kpis.get("test_summary_details", {})
kpi_cols_disease_pos = st.columns(4) # Show 4 key disease positivity rates

# TB GeneXpert Positivity (Example)
...
with kpi_cols_disease_pos[2]:
    ...
    render_kpi_card(f"{hiv_rapid_display_name} Pos.", f"{hiv_pos_rate:.1f}%", "🩸", status="Moderate" if hiv_pos_rate > 2 else "Low") # Lower threshold example for HIV

# Key Drug Stockouts (can stay here or move to supply tab)
...
# Clinic Environment KPIs
if not filtered_iot_df_clinic.empty: # Only show if there's IoT data for the period
...
with tab_tests:
    ...
    if filtered_health_df_clinic.empty:
        ...
        # Detailed test stats are already in clinic_service_kpis
        ...
        if not detailed_test_stats_tab:
            ...
            # Selectbox for test group
            test_group_options_tab = ["All Critical Tests Summary"] + sorted([k for k,v in detailed_test_stats_tab.items() if v.get('total_conducted_conclusive',0) > 0 or v.get('pending_count',0) > 0 or v.get('rejected_count',0) > 0]) # Only show groups with some activity
            if not test_group_options_tab[1:]: # if only "All Critical" is there, means no individual group had activity
                 test_group_options_tab = ["All Critical Tests Summary"] if any(app_config.KEY_TEST_TYPES_FOR_ANALYSIS[original_key].get("critical") for original_key in app_config.KEY_TEST_TYPES_FOR_ANALYSIS) else []
            ...
            if not test_group_options_tab:
                ...
                if selected_test_group_display == "All Critical Tests Summary":
                    ...
                    for group_disp_name, stats in detailed_test_stats_tab.items():
                        # Find the original key for this display name to check if it's critical
                    ...
                    if crit_test_table_data:
                        ...
                        st.caption("No data for critical tests found in this period or no critical tests defined.")
                    ...
                    # Find original key for selected display name to get its config (like target_tat_days, actual test_type names it groups)
                    ...
                    if original_key_for_selected:
                        ...
                        with plot_cols_test_detail_tab[0]: # TAT Trend
                        ...
                        with plot_cols_test_detail_tab[1]: # Volume Trend
                            ...
                            if not df_vol_plot_src.empty:
                                ...
                                if not conducted_vol.empty or not pending_vol.empty:
                                    ...
                                    vol_melt_df = vol_trend_df.melt(id_vars='encounter_date', value_vars=['Conclusive', 'Pending'], var_name='Status', value_name='Count')
                                    st.plotly_chart(plot_bar_chart(vol_melt_df, x_col='encounter_date', y_col='Count', color_col='Status', title=f"Daily Volume Trend", barmode='stack', height=app_config.COMPACT_PLOT_HEIGHT-20), use_container_width=True)
        ...
        # --- Overdue Pending Tests (all tests) ---
        ...
        # Use sample_collection_date if available and more representative
        ...
        if not overdue_df_clinic.empty:
            ...
            def get_specific_overdue_threshold(test_type_disp_name):
                original_key = next((k for k,v_cfg in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items() if v_cfg.get("display_name") == test_type_disp_name), None)
                buffer_days = 2 # Add a small buffer to target TAT for "overdue"
                if original_key: return app_config.KEY_TEST_TYPES_FOR_ANALYSIS[original_key].get('target_tat_days', app_config.OVERDUE_PENDING_TEST_DAYS) + buffer_days
...
with tab_supplies:
    ...
    # Main health_df (not period filtered) for historical rates needed by both forecast types
    if health_data_available and not health_df_clinic_main.empty and all(c in health_df_clinic_main.columns for c in ['item', 'encounter_date', 'item_stock_agg_zone', 'consumption_rate_per_day']):
        ...
        if use_ai_forecast:
            ...
            # Ensure we have the main health_df, not the period-filtered one for AI model's historical view.
            supply_model_ai = SupplyForecastingModel()
            supply_forecast_df = supply_model_ai.forecast_supply_levels_advanced(health_df_clinic_main, forecast_days_out=30) # Forecast based on whole history
        else:
        ...
        if supply_forecast_df is not None and not supply_forecast_df.empty:
            key_drug_items_for_select = sorted(list(supply_forecast_df['item'].unique())) # Show all available items
            if not key_drug_items_for_select:
                 st.info("No forecast data available for any supply items based on historical data.")
            else:
                # Filter items by app_config.KEY_DRUG_SUBSTRINGS_SUPPLY for default selection if list is long
                ...
                if selected_drug_for_forecast:
                    ...
                    if not item_specific_forecast_df.empty:
                        ...
                        current_info_from_forecast = item_specific_forecast_df.iloc[0] # First row of forecast has initial state
                        # Title includes initial stock from the day before forecast starts, and base consumption rate
                        forecast_plot_title = (
                            f"Forecast: {selected_drug_for_forecast}<br>"
                            f"<sup_>Stock at Forecast Start: {current_info_from_forecast.get('current_stock',0):.0f} | "
                            f"Base Daily Use (Historical Avg): {current_info_from_forecast.get('consumption_rate',0):.1f} | "
                        ...
                        # The `plot_annotated_line_chart` expects a Series indexed by date
                        plot_data_series = item_specific_forecast_df.set_index('date')['forecast_days']
                        lower_ci = item_specific_forecast_df.set_index('date').get('lower_ci', None) # For linear model from core
                        upper_ci = item_specific_forecast_df.set_index('date').get('upper_ci', None) # For linear model from core
                        
                        show_ci_plot = (lower_ci is not None and upper_ci is not None and not use_ai_forecast) # Only show CI if from linear and present
                        ...
                        st.plotly_chart(plot_annotated_line_chart(
                            ...
                            height=app_config.DEFAULT_PLOT_HEIGHT + 60, show_anomalies=False # Anomalies not relevant for forecast plot
            ...
            st.warning("Supply forecast data could not be generated. This may be due to missing consumption rates or stock levels in the historical data, or no items matching filters.")
    elif not health_data_available:
        st.warning("Supply forecasts cannot be generated as health records data (needed for consumption rates and stock levels) is unavailable.")
    else: # Health data is available but doesn't have the required columns
        st.error("CRITICAL FOR SUPPLY TAB: Cannot generate supply forecasts. Health data is missing essential columns (item, encounter_date, item_stock_agg_zone, consumption_rate_per_day).")
...
with tab_patients:
    ...
    if not filtered_health_df_clinic.empty:
        ...
        if all(c in filtered_health_df_clinic.columns for c in ['condition', 'encounter_date', 'patient_id']):
            conditions_for_load_chart = app_config.KEY_CONDITIONS_FOR_TRENDS # Use configured key conditions
            patient_load_df_src = filtered_health_df_clinic[
                filtered_health_df_clinic['condition'].isin(conditions_for_load_chart) &
                (filtered_health_df_clinic['patient_id'].astype(str).str.lower() != 'unknown') # ensure patient_id is not 'Unknown'
            ].copy()

            if not patient_load_df_src.empty:
                # Group by encounter_date (daily) and condition, then count unique patients
                ...
                daily_patient_load_summary.rename(columns={'patient_id': 'unique_patients', 'encounter_date':'date'}, inplace=True) # 'date' for plot func
                ...
                if not daily_patient_load_summary.empty:
                    st.plotly_chart(plot_bar_chart(
                        ...
                        text_auto=False # Stacked bar text can get messy
                    ), use_container_width=True)
                else: st.caption("No patient load data for key conditions found in the selected period to display the chart.")
            else: st.caption("No patients with encounters for key conditions recorded in the selected period.")
        else:
            st.info("Patient Load chart cannot be displayed. Missing 'condition', 'encounter_date', or 'patient_id' columns.")

        st.markdown("---"); st.markdown("###### **Flagged Patient Cases for Clinical Review (Selected Period)**")
        # Use the AI-enriched filtered_health_df_clinic
        flagged_patients_clinic_review_df = get_patient_alerts_for_clinic(
            filtered_health_df_clinic, # This now contains AI scores
            risk_threshold_moderate=app_config.RISK_THRESHOLDS['moderate'] # used as base if no other flags
        )
        if flagged_patients_clinic_review_df is not None and not flagged_patients_clinic_review_df.empty:
            st.markdown(f"Found **{len(flagged_patients_clinic_review_df)}** unique patient encounters flagged for review based on AI risk, critical positive tests, overdue tests, or other factors within the period.")
            
            # Display AI scores and other relevant details
            cols_for_alert_table_clinic = ['patient_id', 'encounter_date', 'condition',
                                           ...
                                           'priority_score'] # priority_score from get_patient_alerts_for_clinic combines factors
            ...
            # Sort by the composite priority_score from get_patient_alerts_for_clinic
            ...
            st.dataframe(alerts_display_df_clinic_sorted.head(25), use_container_width=True,
                column_config={
                    ...
                    "priority_score": st.column_config.NumberColumn("Overall Alert Prio.", format="%d", help="Aggregated priority based on all alert factors."),
                    "alert_reason": st.column_config.TextColumn("Alert Reason(s)", width="large", help="Reasons why this patient case is flagged."),
                    "hiv_viral_load_copies_ml": st.column_config.NumberColumn("HIV VL", format="%.0f copies/mL", help="HIV Viral Load if applicable."),
                    "min_spo2_pct": st.column_config.NumberColumn("Min SpO2", format="%d%%", help="Lowest SpO2 if recorded."),
                },
        ...
        st.info("No health data available for the selected period to display Patient Load or generate patient alerts.")


with tab_environment:
    st.subheader("🌿 Clinic Environmental Monitoring - Trends & Details")
    if not filtered_iot_df_clinic.empty:
        env_summary_for_tab = get_clinic_environmental_summary(filtered_iot_df_clinic) # Use period-filtered IoT
        st.markdown(f"""**Current Environmental Alerts (based on latest readings in selected period):**
        ...
        if env_summary_for_tab.get('high_occupancy_alert_latest', False):
            st.warning(f"⚠️ **High Waiting Room Occupancy Detected:** At least one area has latest occupancy > {app_config.TARGET_WAITING_ROOM_OCCUPANCY} persons in the period.")
        ...
        with env_trend_plot_cols[0]:
            if 'avg_co2_ppm' in filtered_iot_df_clinic.columns:
                ...
                else: st.caption("No CO2 trend data available for the selected period.")
        ...
        with env_trend_plot_cols[1]:
            if 'waiting_room_occupancy' in filtered_iot_df_clinic.columns:
                ...
                else: st.caption("No waiting room occupancy trend data available for the selected period.")
        ...
        if all(c in available_latest_cols_env for c in ['timestamp', 'clinic_id', 'room_name']): # Essential for meaningful display
    ...
    elif iot_data_available: # Main IoT has data, but filtered_iot is empty
         st.info("No clinic environmental data found for the selected period.")
    # If iot_data_available is False, the initial st.info message handles it.
