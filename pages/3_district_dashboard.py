...
def get_district_dashboard_data_enriched():
    ...
    # Load base health data and apply AI models *before* enrichment for district aggregates
    ...
    if health_df_raw.empty:
        ...
        # Return structure expected by downstream code, even if parts are empty/None
    ...
    if health_df_ai_enriched.empty and not health_df_raw.empty:
        logger.warning("District Dashboard: AI enrichment failed for health data, using raw for GDF enrichment.")
        health_df_for_enrichment = health_df_raw
    else:
        health_df_for_enrichment = health_df_ai_enriched
    ...
    if zone_gdf_base is None or zone_gdf_base.empty or 'geometry' not in zone_gdf_base.columns:
        logger.error("CRITICAL - District Dashboard: Base zone geographic data (zone_gdf_base) is unusable. Map and zonal analyses will fail.")
        # Return health data (if any) and empty/None for GDF and IoT if base GDF fails
    ...
    # Pass the AI-enriched health_df to the enrichment function
    enriched_zone_gdf_final = enrich_zone_geodata_with_health_aggregates(
        zone_gdf_base,
        health_df_for_enrichment, # Use the AI-enriched version
        iot_df
    )

    if enriched_zone_gdf_final.empty and (zone_gdf_base is not None and not zone_gdf_base.empty):
        ...
        # Ensure critical columns for display even in fallback
        for col_check in ['avg_risk_score', 'population', 'name', 'facility_coverage_score', 'active_tb_cases']:
             if col_check not in zone_gdf_base.columns:
                 zone_gdf_base[col_check] = 0 if col_check != 'name' else "Unknown Zone"
...
# Load all data needed for the District dashboard
...
if district_gdf_main_enriched is None or district_gdf_main_enriched.empty or \
   ...
   district_gdf_main_enriched.geometry.is_empty.all():
    st.error("🚨 **CRITICAL GIS Data Error:** Geographic zone data (enriched GDF) is missing, invalid, or empty. The District Dashboard cannot be rendered effectively. Please verify 'zone_geometries.geojson', 'zone_attributes.csv', and their processing via `enrich_zone_geodata_with_health_aggregates`.")
    logger.critical("District Dashboard HALTED: district_gdf_main_enriched is unusable.")
    # Allow to proceed for sidebar and potentially trend data if health_records_district_main is okay
    # st.stop()
else:
...
# Date range selection for TRENDS (KPIs use the full enriched_gdf which is cross-sectional/latest agg)
all_potential_dates_dist_trend = []
if health_records_district_main is not None and not health_records_district_main.empty and 'encounter_date' in health_records_district_main:
    all_potential_dates_dist_trend.extend(pd.to_datetime(health_records_district_main['encounter_date'], errors='coerce').dropna())
if iot_records_district_main is not None and not iot_records_district_main.empty and 'timestamp' in iot_records_district_main:
...
default_start_val_dist_trends = max_date_for_trends_dist - pd.Timedelta(days=app_config.DEFAULT_DATE_RANGE_DAYS_TREND - 1) # Default to 30 days for trends
...
if health_records_district_main is not None and not health_records_district_main.empty:
    health_records_district_main['encounter_date_obj'] = pd.to_datetime(health_records_district_main['encounter_date']).dt.date
    filtered_health_for_trends_dist = health_records_district_main[
...
if iot_records_district_main is not None and not iot_records_district_main.empty:
    iot_records_district_main['timestamp_date_obj'] = pd.to_datetime(iot_records_district_main['timestamp']).dt.date
    filtered_iot_for_trends_dist = iot_records_district_main[
...
# --- KPIs Section (Based on latest enriched GDF snapshot) ---
...
if district_gdf_main_enriched is not None and not district_gdf_main_enriched.empty:
    district_overall_kpis = get_district_summary_kpis(district_gdf_main_enriched) # Uses enriched GDF
...
if district_gdf_main_enriched is not None and not district_gdf_main_enriched.empty and 'geometry' in district_gdf_main_enriched.columns and not district_gdf_main_enriched.geometry.is_empty.all():
    map_metric_options_config_dist = {
        ...
        "Avg. Zone CO2 (Clinics)": {"col": "zone_avg_co2", "colorscale": "Blues_r", "format": "{:.0f} ppm"}, # Reversed blues for high CO2 is bad
    ...
    # Calculate Population Density if area can be computed
    gdf_for_area_calc = district_gdf_main_enriched.copy()
    if 'population' in gdf_for_area_calc.columns:
        try:
            if gdf_for_area_calc.crs and not gdf_for_area_calc.crs.is_geographic: # Already projected
                gdf_for_area_calc['area_sqkm_calc'] = gdf_for_area_calc.geometry.area / 1_000_000
            elif gdf_for_area_calc.crs and gdf_for_area_calc.crs.is_geographic: # Needs projection for area
            ...
            if 'area_sqkm_calc' in gdf_for_area_calc.columns and gdf_for_area_calc['area_sqkm_calc'].notna().any():
                district_gdf_main_enriched['population_density'] = gdf_for_area_calc.apply(
                    lambda r: (r['population'] / r['area_sqkm_calc']) if pd.notna(r['area_sqkm_calc']) and r['area_sqkm_calc'] > 0 and pd.notna(r['population']) else 0, axis=1
                )
    ...
    if available_map_metrics_for_select:
        ...
        if selected_map_metric_config:
            ...
            # Construct hover_cols ensuring 'name' is always included for tooltip readability
            hover_cols_for_map = ['name', map_val_col, 'population'] # Base cols
            if 'num_clinics' in district_gdf_main_enriched.columns: hover_cols_for_map.append('num_clinics')
            if 'facility_coverage_score' in district_gdf_main_enriched.columns and map_val_col != 'facility_coverage_score':
                 hover_cols_for_map.append('facility_coverage_score') # Add if not the main value
            final_hover_cols_map = list(dict.fromkeys([col for col in hover_cols_for_map if col in district_gdf_main_enriched.columns])) # Unique & existing
            ...
            map_figure = plot_layered_choropleth_map(
                ...
                id_col='zone_id', featureidkey_prefix='properties', # Assumes zone_id in GDF properties if using TopoJSON source in px
        ...
        st.warning("No metrics with valid data are currently available for map visualization in the enriched GeoDataFrame. Ensure data enrichment was successful and relevant columns exist.")
else:
    ...
    if (filtered_health_for_trends_dist.empty and (iot_records_district_main is None or filtered_iot_for_trends_dist.empty)):
        ...
        with cols_disease_trends_dist[0]:
            if not filtered_health_for_trends_dist.empty and 'condition' in filtered_health_for_trends_dist.columns and 'patient_id' in filtered_health_for_trends_dist.columns:
                # Count unique new patients with TB per week
                ...
                # This needs careful definition of "new case". If first encounter for TB is new case:
                ...
                weekly_tb_trend = get_trend_data(new_tb_cases_df, 'patient_id', date_col='encounter_date', period='W', agg_func='count') # count distinct new patients
            ...
            else: st.caption("TB trend data cannot be generated (missing required columns or data).")
        ...
        st.subheader("Population Wellness & Clinic Environmental Trends")
        cols_wellness_env_dist = st.columns(2)
        with cols_wellness_env_dist[0]:
            if not filtered_health_for_trends_dist.empty and 'avg_daily_steps' in filtered_health_for_trends_dist.columns:
                ...
                if not steps_trends_dist.empty:
                    st.plotly_chart(plot_annotated_line_chart(steps_trends_dist, "Weekly Avg. Patient Daily Steps (District)", y_axis_title="Average Steps", target_line=app_config.TARGET_DAILY_STEPS, target_label=f"Target {app_config.TARGET_DAILY_STEPS} Steps", height=app_config.COMPACT_PLOT_HEIGHT), use_container_width=True)
        ...
        with cols_wellness_env_dist[1]:
            if not filtered_iot_for_trends_dist.empty and 'avg_co2_ppm' in filtered_iot_for_trends_dist.columns:
                co2_trends_dist_iot = get_trend_data(filtered_iot_for_trends_dist, 'avg_co2_ppm', date_col='timestamp', period='D', agg_func='mean') # Daily average
                if not co2_trends_dist_iot.empty:
                    st.plotly_chart(plot_annotated_line_chart(co2_trends_dist_iot, "Daily Avg. CO2 (All Monitored Clinics)", y_axis_title="CO2 (ppm)", target_line=app_config.CO2_LEVEL_ALERT_PPM, target_label=f"Alert >{app_config.CO2_LEVEL_ALERT_PPM}ppm", height=app_config.COMPACT_PLOT_HEIGHT), use_container_width=True)
...
with tab_dist_comparison:
    ...
    if district_gdf_main_enriched is not None and not district_gdf_main_enriched.empty and 'geometry' in district_gdf_main_enriched.columns:
        ...
        comp_table_metrics_dict_dist = {
            name: details for name, details in map_metric_options_config_dist.items() # Reuse from map config
            if details["col"] in district_gdf_main_enriched.columns and district_gdf_main_enriched[details["col"]].notna().any()
        }
        
        if comp_table_metrics_dict_dist:
            st.subheader("Zonal Statistics Table")
            cols_for_comp_table_display = ['name'] + [d['col'] for d in comp_table_metrics_dict_dist.values()]
            ...
            # Prepare formats for styling
            style_formats_comp_dist = {
                details["col"]: details.get("format", "{:.1f}") # Default format
                for _, details in comp_table_metrics_dict_dist.items()
                if details["col"] in df_for_comp_table_display.columns
            }
            styler_obj_comp_dist = df_for_comp_table_display.style.format(style_formats_comp_dist)

            # Apply background gradient based on colorscale semantics (higher is bad for _r, good otherwise)
            for metric_display_name, details_style_comp in comp_table_metrics_dict_dist.items():
                ...
                if col_name_to_style in df_for_comp_table_display.columns:
                    ...
                    cmap_gradient = colorscale_name
                    # Heuristic: if '_r' is in colorscale, higher values are worse (redder). If not, higher values are better (greener/bluer).
                    # This is a simplification. Some non-'_r' scales are still 'bad' if high (e.g. Plasma).
                    # Plotly Express colorscales don't directly map to Matplotlib cmap objects perfectly.
                    # We use the provided Plotly scale names.
                    try:
                        styler_obj_comp_dist = styler_obj_comp_dist.background_gradient(subset=[col_name_to_style], cmap=cmap_gradient, axis=0)
                    except Exception as e_style:
                         logger.warning(f"Could not apply background_gradient for {col_name_to_style} with cmap {cmap_gradient}: {e_style}")
            st.dataframe(styler_obj_comp_dist, use_container_width=True, height=min(len(df_for_comp_table_display) * 45 + 60, 650)) # Adjusted height
            ...
            selected_bar_metric_name_dist_comp_viz = st.selectbox(
                ...
                list(comp_table_metrics_dict_dist.keys()),
                key="district_comp_barchart_v8"
            )
            ...
            if selected_bar_details_dist_comp_viz:
                ...
                # text_format_bar = selected_bar_details_dist_comp_viz.get("format", "{:.1f}").replace('{','').replace('}','').split(':')[-1] # Get format specifier
                text_format_bar = selected_bar_details_dist_comp_viz.get("format", ",.1f") # Using default plotly format string style
                sort_asc_bar_viz = "_r" not in selected_bar_details_dist_comp_viz.get("colorscale", "") # Higher is better if not reversed scale
                
                st.plotly_chart(plot_bar_chart(
                    district_gdf_main_enriched, x_col='name', y_col=bar_col_for_comp_viz,
                    ...
                    text_auto=True, text_format=text_format_bar # Pass the format string directly
                ), use_container_width=True)
        else:
            st.info("No metrics available for Zonal Comparison table/chart. Check GDF enrichment.")
    else:
...
with tab_dist_interventions:
    ...
    if district_gdf_main_enriched is not None and not district_gdf_main_enriched.empty and 'geometry' in district_gdf_main_enriched.columns:
        ...
        # Define criteria functions based on columns in enriched GDF
        # These lambda functions expect the GDF as input
        ...
        for name_crit_int, func_crit_int in criteria_lambdas_intervention_dist.items():
            try: # Test if the function can be applied (i.e., columns exist)
                # Apply to a small head or empty df with same columns to check applicability
            ...
            except Exception as e_crit_test:
                logger.debug(f"Intervention criterion '{name_crit_int}' not available, column likely missing or error: {e_crit_test}")
                pass # Criterion cannot be applied

        if not available_criteria_for_intervention_dist:
            st.warning("No intervention criteria can be applied. Relevant data columns may be missing from the enriched zone data or encounter errors during application test.")
        else:
            ...
            selected_criteria_names_interv = st.multiselect(
                ...
                key="district_intervention_criteria_multiselect_v5",
            ...
            if not selected_criteria_names_interv:
                ...
                for crit_name_selected_interv in selected_criteria_names_interv:
                    ...
                    try:
                        ...
                        if isinstance(current_crit_mask_interv, pd.Series) and current_crit_mask_interv.dtype == 'bool':
                            ...
                            logger.warning(f"Intervention criterion '{crit_name_selected_interv}' did not produce a valid boolean Series. Result type: {type(current_crit_mask_interv)}")
                    except Exception as e_crit_apply_interv:
                        logger.error(f"Error applying intervention criterion '{crit_name_selected_interv}': {e_crit_apply_interv}", exc_info=True)
                        st.warning(f"Could not apply criterion: {crit_name_selected_interv}. Error: {str(e_crit_apply_interv)[:100]}...") # Show brief error
                ...
                if not priority_zones_df_for_interv.empty:
                    ...
                    # Define sort order: High risk first, then high prevalence, then low facility coverage
                    sort_by_list_interv_display = []
                    sort_asc_list_interv_display = []
                    ...
                    interv_df_display_sorted_final = priority_zones_df_for_interv.sort_values(
                        by=sort_by_list_interv_display, ascending=sort_asc_list_interv_display
                    ) if sort_by_list_interv_display else priority_zones_df_for_interv
                    
                    st.dataframe(
                        interv_df_display_sorted_final[actual_cols_interv_table_display],
                        use_container_width=True, hide_index=True, height=min(400, len(interv_df_display_sorted_final)*38 + 58),
                        column_config={
        ...
        st.info("Intervention planning insights require successfully loaded and enriched geographic zone data. Please check data sources and processing steps.")
