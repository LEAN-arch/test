...
def render_clinic_epi_module(filtered_health_df_clinic, date_range_display_str):
    ...
    st.header(f"Clinic-Level Epidemiology Analysis {date_range_display_str}") # Corrected to English

    epi_cols_clinic = st.columns(2)
    with epi_cols_clinic[0]:
        st.subheader("Symptom Trends (Weekly)") # Corrected
        if 'patient_reported_symptoms' in filtered_health_df_clinic.columns and filtered_health_df_clinic['patient_reported_symptoms'].notna().any():
            ...
            symptoms_df = symptoms_df[symptoms_df['patient_reported_symptoms'].str.lower() != "unknown"]
            if not symptoms_df.empty:
                ...
                if not symptoms_to_plot.empty:
                    ...
                    fig_symptoms = plot_bar_chart(symptom_trends, x_col='week_start_date', y_col='count', color_col='symptom', title="Weekly Symptom Frequency (Top 5)", barmode='stack', height=app_config.DEFAULT_PLOT_HEIGHT, y_axis_title="Number of Encounters", x_axis_title="Week") # Corrected
                    st.plotly_chart(fig_symptoms, use_container_width=True)
                else: st.caption("Not enough data to show symptom trends (Top 5).")
            else: st.caption("No patient reported symptoms data (excluding 'Unknown').")
        else: st.caption("Patient reported symptoms data not available.")

    with epi_cols_clinic[1]:
        st.subheader("Test Positivity Rate Trends") # Corrected
        ...
        if not malaria_df_trend.empty:
            ...
            if not weekly_malaria_pos_rate.empty:
                st.plotly_chart(plot_annotated_line_chart(weekly_malaria_pos_rate, f"{mal_rdt_display} Positivity Rate (%)", y_axis_title="Positivity Rate (%)", target_line=app_config.TARGET_MALARIA_POSITIVITY_RATE, height=app_config.COMPACT_PLOT_HEIGHT), use_container_width=True)
            else: st.caption(f"No positivity trend data for {mal_rdt_display}.")
        else: st.caption(f"No test data for {mal_rdt_display}.")

    st.markdown("---")
    st.subheader("Demographic Breakdown for Selected Condition") # Corrected
    conditions_for_demog = ["All Conditions"] + sorted(filtered_health_df_clinic['condition'].dropna().unique().tolist())
    selected_condition_demog = st.selectbox("Select Condition for Demographic Analysis:", options=conditions_for_demog, index=0, key="clinic_demog_cond_select_v2_eng") # Corrected
    ...
    new_cases_demog_df = pd.DataFrame() # Initialize
    if 'encounter_date' in condition_df_demog_source.columns and 'patient_id' in condition_df_demog_source.columns and 'condition' in condition_df_demog_source.columns :
        condition_df_demog_source_copy = condition_df_demog_source.copy()
        condition_df_demog_source_copy.sort_values('encounter_date', inplace=True)
        new_cases_demog_df = condition_df_demog_source_copy.drop_duplicates(subset=['patient_id', 'condition'], keep='first')
    ...
    if not new_cases_demog_df.empty:
        ...
        with demog_breakdown_cols_epi[0]:
            if 'age' in new_cases_demog_df.columns and new_cases_demog_df['age'].notna().any():
                ...
                new_cases_demog_df = new_cases_demog_df.copy()
                ...
                if not age_dist_demog.empty: st.plotly_chart(plot_bar_chart(age_dist_demog, 'Age Group', 'New Cases', f"{selected_condition_demog} - New Cases by Age Group", height=300), use_container_width=True) # Corrected
        ...
        with demog_breakdown_cols_epi[1]:
            if 'gender' in new_cases_demog_df.columns and new_cases_demog_df['gender'].notna().any():
                ...
                if not gender_dist_demog.empty: st.plotly_chart(plot_donut_chart(gender_dist_demog, 'Gender', 'New Cases', f"{selected_condition_demog} - New Cases by Gender", height=300), use_container_width=True) # Corrected
                else: st.caption("No data for gender distribution.")
            else: st.caption("Gender data not available.")
    elif selected_condition_demog == "All Conditions" and filtered_health_df_clinic.empty: # This check should refer to condition_df_demog_source or new_cases_demog_df
         st.caption(f"No cases found for 'All Conditions' in this period for demographic breakdown.")
    elif selected_condition_demog != "All Conditions":
         st.caption(f"No cases found for selected condition '{selected_condition_demog}' in this period for demographic breakdown.")

    st.markdown("---")
    st.subheader("Referral Funnel Analysis (Simple)") # Corrected
    if 'referral_status' in filtered_health_df_clinic.columns and filtered_health_df_clinic['referral_status'].notna().any():
        referral_df_funnel = filtered_health_df_clinic[filtered_health_df_clinic['referral_status'].str.lower() != 'n/a'].copy() # handle case insensitivity
        if not referral_df_funnel.empty:
            total_referrals_made = referral_df_funnel['encounter_id'].nunique()
            completed_referrals = referral_df_funnel[referral_df_funnel.get('referral_outcome','').isin(['Completed', 'Service Provided', 'Attended'])]['encounter_id'].nunique() if 'referral_outcome' in referral_df_funnel else 0
            ...
            funnel_data = pd.DataFrame([
                ...
                {'stage': 'Referrals Completed (Outcome)', 'count': completed_referrals},
                {'stage': 'Referrals Still Pending', 'count': pending_referrals},
            ])
            if not funnel_data.empty and funnel_data['count'].sum() > 0:
                 st.plotly_chart(plot_bar_chart(funnel_data, 'stage', 'count', "Referral Statuses", height=350, y_axis_title="Number of Referrals"), use_container_width=True) # Corrected
            else: st.caption("No data for referral funnel.")
        else: st.caption("No referral records (excluding 'N/A') found for analysis.")
