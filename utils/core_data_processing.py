...
def _robust_merge_agg(
    ...
    temp_agg_col_name = f"{target_col_name}_temp_agg_val__{np.random.randint(0,10000)}" # Unique temp col
    
    right_df_renamed = right_df.rename(columns={value_col_in_right_df: temp_agg_col_name})
    
    merged_df = left_df.merge(right_df_renamed[[on_col, temp_agg_col_name]], on=on_col, how='left')
    ...
    try:
        ...
        for col in string_like_cols:
            if col in df.columns:
                ...
                df.loc[df[col].isin(['', 'nan', 'None', 'N/A', '#N/A', 'np.nan']), col] = "Unknown" # Added np.nan as string
            else: df[col] = "Unknown"
...
def load_iot_clinic_environment_data(file_path: str = None) -> pd.DataFrame:
    ...
    try:
        ...
        for col in string_iot_cols: # Ensure these columns exist even if not in CSV
...
def load_zone_data(attributes_path: str = None, geometries_path: str = None) -> Optional[gpd.GeoDataFrame]:
    ...
    try:
        ...
        merged_gdf = zone_geometries_gdf.merge(zone_attributes_df, on="zone_id", how="left", suffixes=('_geom', '')) # Keep geom name if attr has same col name
        for col_attr in zone_attributes_df.columns:
            if f"{col_attr}_geom" in merged_gdf.columns and col_attr in merged_gdf.columns and col_attr != 'zone_id':
                 merged_gdf[col_attr] = merged_gdf[col_attr].fillna(merged_gdf[f"{col_attr}_geom"]) # Fill self with geom if self is NaN
                 merged_gdf.drop(columns=[f"{col_attr}_geom"], inplace=True, errors='ignore')
        if 'geometry' not in merged_gdf.columns and 'geometry_geom' in merged_gdf.columns : merged_gdf.rename(columns={'geometry_geom':'geometry'}, inplace=True)
        if not merged_gdf.geometry.name == 'geometry' and 'geometry' in merged_gdf.columns: merged_gdf = merged_gdf.set_geometry('geometry')
        ...
        for rz_col in ['zone_id', 'name', 'population', 'geometry', 'num_clinics', 'socio_economic_index']:
            if rz_col not in merged_gdf.columns:
                ...
                elif rz_col == 'name': merged_gdf[rz_col] = "Unknown Zone " + merged_gdf['zone_id'].astype(str) if 'zone_id' in merged_gdf else "Unknown Zone"
                elif rz_col != 'geometry': merged_gdf[rz_col] = None # String cols to None
        for num_col in ['population', 'socio_economic_index', 'num_clinics', 'avg_travel_time_clinic_min']:
            if num_col in merged_gdf.columns: merged_gdf[num_col] = _convert_to_numeric(merged_gdf[num_col], 0 if num_col in ['population', 'num_clinics'] else (0.5 if num_col=='socio_economic_index' else 30))
...
def enrich_zone_geodata_with_health_aggregates(
    ...
    if zone_gdf is None or zone_gdf.empty or 'zone_id' not in zone_gdf.columns:
        return zone_gdf if zone_gdf is not None else gpd.GeoDataFrame(columns=['zone_id', 'geometry'], crs=app_config.DEFAULT_CRS)
    ...
    if health_df is not None and not health_df.empty and 'zone_id' in health_df.columns:
        ...
        key_conditions_burden = ['TB', 'Malaria', 'HIV-Positive', 'Pneumonia'] # Example
        ...
        if 'referral_status' in health_df_for_agg.columns:
            enriched = _robust_merge_agg(enriched, health_df_for_agg[health_df_for_agg['referral_status'].notna() & (health_df_for_agg['referral_status'] != 'N/A') & (health_df_for_agg['referral_status'] != 'Unknown')].groupby('zone_id')['encounter_id'].nunique().reset_index(), 'total_referrals_made')
        ...
        if crit_test_keys:
            ...
            if not tat_df.empty:
                ...
                def _check_tat_met(row_tat): # Renamed parameter to avoid conflict
                    cfg = app_config.KEY_TEST_TYPES_FOR_ANALYSIS.get(row_tat['test_type'])
                    return row_tat['test_turnaround_days'] <= (cfg['target_tat_days'] if cfg and 'target_tat_days' in cfg else app_config.TARGET_TEST_TURNAROUND_DAYS)
                tat_df['tat_met_flag'] = tat_df.apply(_check_tat_met, axis=1)
                perc_met_agg = tat_df.groupby('zone_id')['tat_met_flag'].mean().reset_index()
                perc_met_agg.iloc[:, 1] *= 100 # Modify the value column (second col) directly
...
def get_overall_kpis(health_df: pd.DataFrame, date_filter_start: Optional[str] = None, date_filter_end: Optional[str] = None) -> Dict[str, Any]:
    ...
    df.dropna(subset=['encounter_date'], inplace=True) # Remove rows where date conversion failed for filtering
    ...
    # Use display names for consistency in matching KEY_TEST_TYPES_FOR_ANALYSIS
    mal_rdt_disp_name = app_config.KEY_TEST_TYPES_FOR_ANALYSIS.get("RDT-Malaria", {}).get("display_name", "Malaria RDT")
    malaria_rdt_df = df[(df['test_type'] == mal_rdt_disp_name) & (~df['test_result'].isin(["Pending", "Rejected Sample", "Unknown"]))]
    if not malaria_rdt_df.empty: kpis["malaria_rdt_positive_rate_period"] = (malaria_rdt_df[malaria_rdt_df['test_result'] == 'Positive'].shape[0] / len(malaria_rdt_df)) * 100

    hiv_rapid_disp_name = app_config.KEY_TEST_TYPES_FOR_ANALYSIS.get("HIV-Rapid", {}).get("display_name", "HIV Rapid Test")
    hiv_rapid_df = df[(df['test_type'] == hiv_rapid_disp_name) & (~df['test_result'].isin(["Pending", "Rejected Sample", "Unknown"]))]
    if not hiv_rapid_df.empty: kpis["hiv_rapid_positive_rate_period"] = (hiv_rapid_df[hiv_rapid_df['test_result'] == 'Positive'].shape[0] / len(hiv_rapid_df)) * 100
    
    if 'item' in df.columns and 'item_stock_agg_zone' in df.columns and 'consumption_rate_per_day' in df.columns:
        supply_df = df.drop_duplicates(subset=['item', 'zone_id'], keep='last')
...
def get_chw_summary(health_df_daily: pd.DataFrame) -> Dict[str, Any]:
    ...
    chw_enc_df = health_df_daily.copy() # Assuming it's already filtered for CHW relevant encounters for the day
    if 'chw_visit' in chw_enc_df.columns and chw_enc_df['chw_visit'].dtype in [np.int64, np.float64] and chw_enc_df['chw_visit'].sum() > 0 :
        chw_enc_df = chw_enc_df[chw_enc_df['chw_visit'] == 1]
    elif 'encounter_type' in chw_enc_df.columns and chw_enc_df['encounter_type'].str.contains("CHW", case=False, na=False).any():
        chw_enc_df = chw_enc_df[chw_enc_df['encounter_type'].str.contains("CHW", case=False, na=False)]
    if chw_enc_df.empty: return summary

    summary["visits_today"] = chw_enc_df['patient_id'].nunique()
    if 'condition' in chw_enc_df.columns and 'referral_reason' in chw_enc_df.columns and 'referral_status' in chw_enc_df.columns:
        ...
        summary["sti_symptomatic_referrals_today"] = chw_enc_df[(chw_enc_df['condition'].str.contains("STI", case=False, na=False)) & (chw_enc_df['patient_reported_symptoms'] != "Unknown") & (chw_enc_df['referral_status'] == 'Pending')]['patient_id'].nunique()
    ...
    temp_col = 'vital_signs_temperature_celsius' if 'vital_signs_temperature_celsius' in chw_enc_df.columns and chw_enc_df['vital_signs_temperature_celsius'].notna().any() else 'max_skin_temp_celsius'
    if temp_col in chw_enc_df.columns: summary["patients_fever_visited_today"] = chw_enc_df[chw_enc_df[temp_col] >= app_config.SKIN_TEMP_FEVER_THRESHOLD_C]['patient_id'].nunique()
...
def get_patient_alerts_for_chw(health_df_daily: pd.DataFrame, risk_threshold_moderate: int = app_config.RISK_THRESHOLDS['chw_alert_moderate'], risk_threshold_high: int = app_config.RISK_THRESHOLDS['chw_alert_high']) -> pd.DataFrame:
    ...
    required_alert_cols = ['patient_id', 'ai_risk_score', 'ai_followup_priority_score', 'min_spo2_pct', 'vital_signs_temperature_celsius', 'max_skin_temp_celsius', 'condition', 'referral_status', 'fall_detected_today', 'encounter_date']
    for col in required_alert_cols:
        if col not in df_alerts.columns: df_alerts[col] = np.nan if 'score' in col or 'pct' in col or 'temp' in col else ("Unknown" if col in ['condition','referral_status'] else (pd.NaT if col == 'encounter_date' else 0) )
    
    high_ai_prio = df_alerts[df_alerts['ai_followup_priority_score'] >= 80]
    for _, r in high_ai_prio.iterrows(): alerts.append({**r.to_dict(), 'alert_reason': "High AI Follow-up Priority", 'priority_score': r['ai_followup_priority_score']})
    crit_spo2 = df_alerts[df_alerts['min_spo2_pct'] < app_config.SPO2_CRITICAL_THRESHOLD_PCT]
    for _, r in crit_spo2.iterrows(): alerts.append({**r.to_dict(), 'alert_reason': f"Critical SpO2 ({r['min_spo2_pct']}%)", 'priority_score': 90 + (app_config.SPO2_CRITICAL_THRESHOLD_PCT - r['min_spo2_pct'])})
    temp_c = 'vital_signs_temperature_celsius' if df_alerts['vital_signs_temperature_celsius'].notna().any() else 'max_skin_temp_celsius'
    high_fever = df_alerts[df_alerts[temp_c] >= app_config.SKIN_TEMP_FEVER_THRESHOLD_C + 1.0]
    for _, r in high_fever.iterrows(): alerts.append({**r.to_dict(), 'alert_reason': f"High Fever ({r[temp_c]}°C)", 'priority_score': 85 + (r[temp_c] - (app_config.SKIN_TEMP_FEVER_THRESHOLD_C + 1.0))})
    falls_df = df_alerts[df_alerts['fall_detected_today'] > 0]
    for _, r in falls_df.iterrows(): alerts.append({**r.to_dict(), 'alert_reason': "Fall Detected", 'priority_score': 88})
    high_risk_gen = df_alerts[df_alerts['ai_risk_score'] >= risk_threshold_high]
    for _, r in high_risk_gen.iterrows(): alerts.append({**r.to_dict(), 'alert_reason': "High General AI Risk", 'priority_score': r['ai_risk_score']})
    crit_cond_ref = df_alerts[(df_alerts['condition'].isin(app_config.KEY_CONDITIONS_FOR_TRENDS[:4])) & (df_alerts['referral_status'] == 'Pending')]
    for _, r in crit_cond_ref.iterrows(): alerts.append({**r.to_dict(), 'alert_reason': f"Pending Referral for {r['condition']}", 'priority_score': 70})
    mod_risk = df_alerts[(df_alerts['ai_risk_score'] >= risk_threshold_moderate) & (df_alerts['ai_risk_score'] < risk_threshold_high)]
    for _, r in mod_risk.iterrows(): alerts.append({**r.to_dict(), 'alert_reason': "Moderate AI Risk", 'priority_score': r['ai_risk_score']})
        
    if not alerts: return pd.DataFrame()
    alert_df = pd.DataFrame(alerts).drop_duplicates(subset=['patient_id', 'alert_reason', pd.to_datetime(health_df_daily['encounter_date']).dt.date])
...
def get_clinic_summary(health_df_period: pd.DataFrame) -> Dict[str, Any]:
    ...
    test_cols_req = ['test_type', 'test_result', 'test_turnaround_days', 'sample_status', 'rejection_reason', 'encounter_date'] # Added encounter_date
    for col in test_cols_req:
        if col not in df.columns: df[col] = np.nan if col in ['test_turnaround_days'] else ("Unknown" if col != 'encounter_date' else pd.NaT)

    conclusive_df = df[~df['test_result'].isin(['Pending', 'Rejected Sample', 'Unknown', 'N/A', 'nan','Indeterminate']) & df['test_turnaround_days'].notna()]
    all_proc_samples_df = df[~df['sample_status'].isin(['Pending', 'Unknown', 'N/A', 'nan'])]
    ...
    if not crit_conclusive.empty:
        def _check_tat_met_overall(r):
            key = next((k for k,cfg in crit_test_cfgs.items() if cfg['display_name'] == r['test_type']), None)
            return r['test_turnaround_days'] <= (crit_test_cfgs[key]['target_tat_days'] if key else app_config.TARGET_TEST_TURNAROUND_DAYS)
        crit_conclusive['tat_met'] = crit_conclusive.apply(_check_tat_met_overall, axis=1)
    ...
    if not all_proc_samples_df.empty: summary["sample_rejection_rate"] = (all_proc_samples_df[all_proc_samples_df['sample_status'] == 'Rejected'].shape[0] / len(all_proc_samples_df)) * 100
    ...
    for orig_key, cfg_props in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items():
        ...
        actual_types = cfg_props.get("types_in_group", [orig_key]) # Assuming test_type in data matches orig_key or is in types_in_group
        grp_df = df[df['test_type'].isin(actual_types if isinstance(actual_types, list) else [actual_types])]
        if grp_df.empty: continue; stats = {}
        grp_concl = grp_df[~grp_df['test_result'].isin(['Pending', 'Rejected Sample', 'Unknown', 'N/A', 'nan','Indeterminate']) & grp_df['test_turnaround_days'].notna()]
        if not grp_concl.empty:
            stats["positive_rate"] = (grp_concl[grp_concl['test_result'] == 'Positive'].shape[0] / len(grp_concl)) * 100
            ...
            grp_concl_copy = grp_concl.copy(); grp_concl_copy.loc[:, 'tat_met_specific'] = grp_concl_copy['test_turnaround_days'] <= tgt_tat
            stats["perc_met_tat_target"] = grp_concl_copy['tat_met_specific'].mean() * 100
...
def get_patient_alerts_for_clinic(health_df_period: pd.DataFrame, risk_threshold_moderate: int = app_config.RISK_THRESHOLDS['moderate']) -> pd.DataFrame:
    ...
    for col in alert_cols:
        if col not in df_alerts.columns: df_alerts[col] = np.nan if 'score' in col or 'load' in col or 'pct' in col or 'temp' in col else ("Unknown" if col not in ['encounter_date'] else pd.NaT)
    
    high_risk_ai = df_alerts[(df_alerts['ai_risk_score'] >= risk_threshold_moderate) | (df_alerts['ai_followup_priority_score'] >= 75)]
    for _, r in high_risk_ai.iterrows(): alerts_data.append({**r.to_dict(), 'alert_reason': f"High AI Risk/Priority (Risk:{r.get('ai_risk_score',0):.0f}, Prio:{r.get('ai_followup_priority_score',0):.0f})", 'priority_score': max(r.get('ai_risk_score',0), r.get('ai_followup_priority_score',0))})
    crit_pos = df_alerts[(df_alerts['test_type'].isin([cfg['display_name'] for _, cfg in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items() if cfg.get('critical')])) & (df_alerts['test_result'] == 'Positive')]
    ...
    if 'hiv_viral_load_copies_ml' in df_alerts and df_alerts['hiv_viral_load_copies_ml'].notna().any():
        high_vl = df_alerts[df_alerts['hiv_viral_load_copies_ml'] > 1000]
        for _, r in high_vl.iterrows(): alerts_data.append({**r.to_dict(), 'alert_reason': f"High HIV Viral Load ({r['hiv_viral_load_copies_ml']:.0f})", 'priority_score': 90})
    crit_spo2_clinic = df_alerts[df_alerts['min_spo2_pct'] < app_config.SPO2_CRITICAL_THRESHOLD_PCT]
    for _, r in crit_spo2_clinic.iterrows(): alerts_data.append({**r.to_dict(), 'alert_reason': f"Critically Low SpO2 ({r['min_spo2_pct']:.0f}%)", 'priority_score': 92})
    
    date_col_overdue = 'sample_registered_lab_date' if 'sample_registered_lab_date' in df_alerts.columns and df_alerts['sample_registered_lab_date'].notna().any() else ('sample_collection_date' if 'sample_collection_date' in df_alerts.columns and df_alerts['sample_collection_date'].notna().any() else 'encounter_date')
    if date_col_overdue in df_alerts.columns and df_alerts[date_col_overdue].notna().any():
        pending_df = df_alerts[(df_alerts['test_result'] == 'Pending') & (df_alerts['test_type'].isin([cfg['display_name'] for _, cfg in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items() if cfg.get('critical')])) & (df_alerts[date_col_overdue].notna())].copy()
        if not pending_df.empty:
            pending_df['days_pending'] = (pd.Timestamp('today').normalize() - pd.to_datetime(pending_df[date_col_overdue])).dt.days
            def get_overdue_thresh(ttype_disp):
                k = next((key for key,v in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items() if v['display_name']==ttype_disp), None)
                return (app_config.KEY_TEST_TYPES_FOR_ANALYSIS[k]['target_tat_days'] if k else app_config.OVERDUE_PENDING_TEST_DAYS) + 2
            pending_df['overdue_thresh_days'] = pending_df['test_type'].apply(get_overdue_thresh)
            overdue_crit = pending_df[pending_df['days_pending'] > pending_df['overdue_thresh_days']]
            for _, r in overdue_crit.iterrows(): alerts_data.append({**r.to_dict(), 'alert_reason': f"Overdue Pending: {r['test_type']} ({r['days_pending']:.0f}d)", 'priority_score': 75 + min(r['days_pending'] - r['overdue_thresh_days'], 10)})
    ...
    alerts_df.dropna(subset=['patient_id', 'encounter_date'], inplace=True)
    ...
    def aggregate_alerts(group):
        ...
        first_row['alert_reason'] = "; ".join(alert_reasons_str) if alert_reasons_str else "General Alert"
        first_row['priority_score'] = group['priority_score'].max()
        # Include all other columns from the first row of the group
        for col in group.columns:
            if col not in ['alert_reason', 'priority_score'] and col not in first_row: # Avoid overwriting already set keys
                first_row[col] = group[col].iloc[0]
        return pd.Series(first_row)

    # Apply and reset index, handle potential empty groups after filtering
    # Make sure patient_id and encounter_date_obj are valid for grouping.
    valid_group_keys = alerts_df['patient_id'].notna() & alerts_df['encounter_date_obj'].notna()
    if not valid_group_keys.any(): return pd.DataFrame(columns=alerts_df.columns) # No valid groups
    
    final_alerts_df = alerts_df[valid_group_keys].groupby(['patient_id', 'encounter_date_obj']).apply(aggregate_alerts).reset_index()
    ...
    sort_date_col = 'encounter_date' if 'encounter_date' in final_alerts_df.columns else 'encounter_date_obj'
    if sort_date_col not in final_alerts_df.columns: sort_date_col = 'encounter_date_obj' # Ensure it exists
...
def get_district_summary_kpis(enriched_zone_gdf: gpd.GeoDataFrame) -> Dict[str, Any]:
    ...
    for col in num_cols: gdf[col] = _convert_to_numeric(gdf[col], 0) if col in gdf.columns else 0.0
    ...
    if kpis["total_population_district"] > 0:
        ...
        kpis["key_infection_prevalence_district_per_1000"] = (gdf['total_active_key_infections'].sum() / kpis["total_population_district"]) * 1000
    else: # Fallbacks if total_population_district is 0
...
def get_supply_forecast_data(health_df: pd.DataFrame, forecast_days_out: int = 30, item_filter_list: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    if health_df is None or health_df.empty or not all(c in health_df.columns for c in ['item', 'encounter_date', 'item_stock_agg_zone', 'consumption_rate_per_day']):
        return pd.DataFrame(columns=['item','date','current_stock','consumption_rate','forecast_stock','forecast_days','estimated_stockout_date','lower_ci','upper_ci','initial_days_supply'])
    ...
    if supply_status_df.empty: return pd.DataFrame(columns=['item','date','current_stock','consumption_rate','forecast_stock','forecast_days','estimated_stockout_date','lower_ci','upper_ci','initial_days_supply'])
    ...
    for _, row in supply_status_df.iterrows():
        ...
        if pd.isna(cur_stock) or pd.isna(cons_rate) or cur_stock < 0: continue
        forecast_dates = pd.date_range(start=last_date + pd.Timedelta(days=1), periods=forecast_days_out, freq='D')
        cur_fc_stock, est_stockout_date = cur_stock, pd.NaT
        days_rem_start = (cur_stock / cons_rate) if cons_rate > 0 else np.inf
        for i, fc_date in enumerate(forecast_dates):
            days_out = i + 1; cur_fc_stock = cur_stock - (cons_rate * days_out)
            days_of_supply_fc = (cur_fc_stock / cons_rate) if cons_rate > 0 else (np.inf if cur_fc_stock > 0 else 0)
            cons_std_factor = 0.15; lower_cons = cons_rate * (1+cons_std_factor); upper_cons = max(0.1, cons_rate * (1-cons_std_factor))
            lower_ci_stock = cur_stock - (lower_cons*days_out); upper_ci_stock = cur_stock - (upper_cons*days_out)
            lower_ci_days = (lower_ci_stock/lower_cons) if lower_cons > 0 else (np.inf if lower_ci_stock > 0 else 0)
            upper_ci_days = (upper_ci_stock/upper_cons) if upper_cons > 0 else (np.inf if upper_ci_stock > 0 else 0)
            if cur_fc_stock <= 0 and pd.isna(est_stockout_date): est_stockout_date = last_date + pd.to_timedelta(cur_stock/cons_rate if cons_rate > 0 else np.inf, unit='D')
            forecasts.append({'item': item, 'date': fc_date, 'current_stock': cur_stock, 'consumption_rate': cons_rate, 'forecast_stock': max(0,cur_fc_stock), 'forecast_days': max(0,days_of_supply_fc), 'estimated_stockout_date': est_stockout_date, 'lower_ci': max(0,lower_ci_days), 'upper_ci': max(0,upper_ci_days), 'initial_days_supply': days_rem_start})
        if pd.isna(est_stockout_date) and cons_rate > 0 : final_est_stockout = last_date + pd.to_timedelta(cur_stock / cons_rate, unit='D')
        elif pd.isna(est_stockout_date) : final_est_stockout = pd.NaT
        else: final_est_stockout = est_stockout_date # Use already set one if it happened within forecast
        for entry in forecasts: # Update all if still NaT
            if entry['item'] == item and pd.isna(entry['estimated_stockout_date']): entry['estimated_stockout_date'] = final_est_stockout
    if not forecasts: return pd.DataFrame(columns=['item','date','current_stock','consumption_rate','forecast_stock','forecast_days','estimated_stockout_date','lower_ci','upper_ci','initial_days_supply'])
