...
def hash_geodataframe(gdf: gpd.GeoDataFrame) -> Optional[str]:
    ...
    try:
        geom_col_name = gdf.geometry.name if hasattr(gdf, 'geometry') else 'geometry' # Default if not active
        
        # Determine non-geometry columns safely
        if geom_col_name in gdf.columns and hasattr(gdf[geom_col_name], 'is_empty') and not gdf[geom_col_name].is_empty.all():
            ...
            geom_hash = pd.util.hash_array(gdf.geometry.to_wkt().values).sum() if hasattr(gdf.geometry, 'to_wkt') else 0
...
def _robust_merge_agg(
    ...
    # Preserve index of left_df if it's meaningful
    left_df_index_name = left_df.index.name
    left_df_reset = left_df.reset_index()
    ...
    if left_df_index_name: # Restore index if it existed
      merged_df.set_index(left_df_index_name, inplace=True)
    elif 'index' in merged_df.columns and left_df_index_name is None and 'index' not in left_df.columns : # if reset_index added 'index'
...
def load_health_records(file_path: str = None) -> pd.DataFrame:
    ...
    try:
        ...
        for col in numeric_cols:
            ...
            else: df[col] = np.nan # Add missing numeric columns as NaN
        ...
        for col in string_like_cols:
            ...
            else: df[col] = "Unknown" # Add missing string columns as "Unknown"
        required_cols = ['patient_id', 'encounter_date', 'condition', 'test_type', 'test_result', 'zone_id', 'ai_risk_score'] # Ensure these base ones
        for r_col in required_cols:
            if r_col not in df.columns:
                logger.warning(f"Core required column '{r_col}' not found in health records. Adding as empty.")
...
def load_zone_data(attributes_path: str = None, geometries_path: str = None) -> Optional[gpd.GeoDataFrame]:
    ...
    try:
        ...
        if 'geometry' in merged_gdf.columns and hasattr(merged_gdf, 'geometry') and merged_gdf.geometry.name != 'geometry': merged_gdf = merged_gdf.set_geometry('geometry', inplace=False)
...
def enrich_zone_geodata_with_health_aggregates(
    ...
    if health_df is not None and not health_df.empty and 'zone_id' in health_df.columns:
        ...
        crit_test_keys_enrich = [k for k, v in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items() if v.get("critical", False)]
        if crit_test_keys_enrich:
            ...
            if not tat_df_enrich.empty:
                ...
                def _check_tat_met_enrich_local(row_tat_enrich_local):
                    cfg_local = app_config.KEY_TEST_TYPES_FOR_ANALYSIS.get(row_tat_enrich_local['test_type'])
                ...
                # _robust_merge_agg expects second column to be the value, so rename if needed.
                # However, our robust_merge_agg picks first non-'on_col' column. Let's ensure it picks right one if names are not standard.
                if 'tat_met_flag' not in perc_met_agg_enrich.columns and len(perc_met_agg_enrich.columns) > 1:
                     perc_met_agg_enrich.rename(columns={perc_met_agg_enrich.columns[1]: 'value_for_merge'}, inplace=True) # Temp name for merge logic
                elif 'tat_met_flag' in perc_met_agg_enrich.columns:
                     perc_met_agg_enrich.rename(columns={'tat_met_flag': 'value_for_merge'}, inplace=True)


                enriched = _robust_merge_agg(enriched, perc_met_agg_enrich, 'perc_critical_tests_tat_met')
                if 'perc_critical_tests_tat_met' in enriched.columns: # After merge, scale by 100 if it's a proportion
...
def get_overall_kpis(health_df: pd.DataFrame, date_filter_start: Optional[str] = None, date_filter_end: Optional[str] = None) -> Dict[str, Any]:
    ...
    mal_rdt_key_cfg = app_config.KEY_TEST_TYPES_FOR_ANALYSIS.get("RDT-Malaria", {})
    mal_rdt_name_in_data = mal_rdt_key_cfg.get("display_name", "RDT-Malaria") # This should match test_type in CSV after cleaning, OR the actual key 'RDT-Malaria' itself
    
    # Assuming test_type column in CSV aligns with original keys of KEY_TEST_TYPES_FOR_ANALYSIS
    # (e.g., 'RDT-Malaria', 'Sputum-GeneXpert', not their display_names)
    malaria_rdt_df = df[(df['test_type'] == "RDT-Malaria") & (~df['test_result'].isin(["Pending", "Rejected Sample", "Unknown"]))]
    ...
    hiv_rapid_df = df[(df['test_type'] == "HIV-Rapid") & (~df['test_result'].isin(["Pending", "Rejected Sample", "Unknown"]))]
    ...
    if 'item' in df.columns and 'item_stock_agg_zone' in df.columns and 'consumption_rate_per_day' in df.columns:
        supply_df = df.sort_values('encounter_date').drop_duplicates(subset=['item', 'zone_id'], keep='last') # zone_id might not be relevant if stock is central
...
def get_chw_summary(health_df_daily: pd.DataFrame) -> Dict[str, Any]:
    ...
    if 'chw_visit' in chw_enc_df.columns and chw_enc_df['chw_visit'].dtype in [np.int64, np.float64, int, float] and chw_enc_df['chw_visit'].sum(skipna=True) > 0 : # check sum>0
    ...
    if not is_chw_encounter_identified and not chw_enc_df.empty : pass # Assume all rows relevant if no specific filter applied
    elif chw_enc_df.empty and is_chw_encounter_identified: return summary # Filtered to empty based on CHW flags
    elif chw_enc_df.empty: return summary # Original was empty
...
def get_patient_alerts_for_chw(health_df_daily: pd.DataFrame, risk_threshold_moderate: int = app_config.RISK_THRESHOLDS['chw_alert_moderate'], risk_threshold_high: int = app_config.RISK_THRESHOLDS['chw_alert_high']) -> pd.DataFrame:
    ...
    for col in alert_cols_chw: # Ensure all columns exist, filling with appropriate defaults
        if col not in df_alerts.columns: df_alerts[col] = np.nan if col in ['ai_risk_score','ai_followup_priority_score','min_spo2_pct','vital_signs_temperature_celsius','max_skin_temp_celsius','fall_detected_today'] else ("Unknown" if col in ['condition','referral_status','patient_id'] else pd.NaT) # ensure patient_id also string unknown
    
    if df_alerts['ai_followup_priority_score'].notna().any(): # Check for NaN before comparison
    ...
    if 'fall_detected_today' in df_alerts.columns and df_alerts['fall_detected_today'].notna().any(): # Check existence
    ...
    if 'ai_risk_score' in df_alerts.columns and df_alerts['ai_risk_score'].notna().any(): # Check existence
    ...
    if not alerts: return pd.DataFrame(columns=alert_cols_chw + ['alert_reason', 'priority_score']) # Return empty df with expected cols
    alert_df = pd.DataFrame(alerts)
    alert_df['encounter_date'] = pd.to_datetime(alert_df['encounter_date'], errors='coerce') # ensure datetime for next step
    if 'encounter_date' in alert_df.columns and alert_df['encounter_date'].notna().any(): # Only if valid dates exist for this
        alert_df['encounter_date_obj_for_dedup'] = alert_df['encounter_date'].dt.date
        alert_df.drop_duplicates(subset=['patient_id', 'alert_reason', 'encounter_date_obj_for_dedup'], inplace=True, keep='first') # keep first based on whatever order rows were generated
...
def get_clinic_summary(health_df_period: pd.DataFrame) -> Dict[str, Any]:
    ...
    critical_test_keys_in_data = [k for k in crit_test_cfgs.keys() if k in df['test_type'].unique()] # Test types that are critical AND exist in data
    ...
    if not critical_conclusive_df.empty:
        def _check_tat_met_overall_clinic_final(r):
            test_cfg = app_config.KEY_TEST_TYPES_FOR_ANALYSIS.get(r['test_type'])
            target_tat = test_cfg['target_tat_days'] if test_cfg and 'target_tat_days' in test_cfg else app_config.TARGET_TEST_TURNAROUND_DAYS
            return r['test_turnaround_days'] <= target_tat
        critical_conclusive_df.loc[:, 'tat_met'] = critical_conclusive_df.apply(_check_tat_met_overall_clinic_final, axis=1)
    ...
    for original_key_from_config, cfg_props in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items():
        ...
        if not grp_concl.empty:
            ...
            stats["positive_rate"] = (positive_cases_in_concl / len(grp_concl)) * 100 if len(grp_concl) > 0 else 0.0
...
def get_patient_alerts_for_clinic(health_df_period: pd.DataFrame, risk_threshold_moderate: int = app_config.RISK_THRESHOLDS['moderate']) -> pd.DataFrame:
    ...
    # High AI Risk/Priority
    if df_alerts['ai_risk_score'].notna().any() or df_alerts['ai_followup_priority_score'].notna().any(): # Ensure columns were not all NaNs
        for _, r in df_alerts[(df_alerts.get('ai_risk_score', pd.Series(dtype=float)) >= risk_threshold_moderate) | (df_alerts.get('ai_followup_priority_score', pd.Series(dtype=float)) >= 75)].iterrows(): alerts_data.append({**r.to_dict(), 'alert_reason': f"High AI Risk/Prio (Risk:{r.get('ai_risk_score',0):.0f}, Prio:{r.get('ai_followup_priority_score',0):.0f})", 'priority_score': max(r.get('ai_risk_score',0), r.get('ai_followup_priority_score',0))})
    # Critical Positive Tests
    crit_test_disp_names_alert = [v['display_name'] for k,v in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items() if v.get("critical")] # Use display names for test_type if data has it
    # Assuming test_type column stores the *original keys* (e.g., "RDT-Malaria", not "Malaria RDT")
    crit_test_keys_alert = [k for k,v in app_config.KEY_TEST_TYPES_FOR_ANALYSIS.items() if v.get("critical")]
    for _, r in df_alerts[(df_alerts['test_type'].isin(crit_test_keys_alert)) & (df_alerts['test_result'] == 'Positive')].iterrows(): alerts_data.append({**r.to_dict(), 'alert_reason': f"Critical Positive: {r['test_type']}", 'priority_score': 85})
    # High HIV Viral Load
    ...
    # Overdue Pending Critical Tests
    date_col_for_overdue_calc = 'sample_collection_date' if 'sample_collection_date' in df_alerts.columns and df_alerts['sample_collection_date'].notna().any() else 'encounter_date'
    if date_col_for_overdue_calc in df_alerts.columns and df_alerts[date_col_for_overdue_calc].notna().any():
        pending_df_alerts = df_alerts[(df_alerts['test_result'] == 'Pending') & (df_alerts['test_type'].isin(crit_test_keys_alert)) & (df_alerts[date_col_for_overdue_calc].notna())].copy()
        if not pending_df_alerts.empty:
            pending_df_alerts.loc[:, 'days_pending'] = (pd.Timestamp('today').normalize() - pd.to_datetime(pending_df_alerts[date_col_for_overdue_calc], errors='coerce')).dt.days
            def get_overdue_thresh_alert(test_type_key_from_data):
                test_cfg = app_config.KEY_TEST_TYPES_FOR_ANALYSIS.get(test_type_key_from_data)
                buffer = 2
                return (test_cfg['target_tat_days'] if test_cfg and 'target_tat_days' in test_cfg else app_config.OVERDUE_PENDING_TEST_DAYS) + buffer
            pending_df_alerts.loc[:, 'overdue_thresh_days'] = pending_df_alerts['test_type'].apply(get_overdue_thresh_alert) # Apply to test_type column which holds original keys
            overdue_crit_alerts = pending_df_alerts[pending_df_alerts['days_pending'].notna() & (pending_df_alerts['days_pending'] > pending_df_alerts['overdue_thresh_days'])]
            for _, r in overdue_crit_alerts.iterrows(): alerts_data.append({**r.to_dict(), 'alert_reason': f"Overdue Pending: {r['test_type']} ({r.get('days_pending',0):.0f}d)", 'priority_score': 75 + min(r.get('days_pending',0) - r.get('overdue_thresh_days',0), 10)})
            
    if not alerts_data: return pd.DataFrame(columns=alert_cols_clinic + ['alert_reason', 'priority_score'])
    alerts_df_agg = pd.DataFrame(alerts_data)
    alerts_df_agg['alert_reason'] = alerts_df_agg['alert_reason'].astype(str).fillna("Unknown Alert")
    alerts_df_agg['encounter_date'] = pd.to_datetime(alerts_df_agg['encounter_date'], errors='coerce')
    ...
    def aggregate_alerts_clinic_final(group): # Final refined version
        first_row = group.iloc[0]
        aggregated_reasons = "; ".join(sorted(list(pd.Series([str(r) for r in group['alert_reason'] if pd.notna(r)]).unique()))) # Sorted unique reasons
        max_priority = group['priority_score'].max() if pd.notna(group['priority_score']).any() else 0
        # Create a new dictionary for the aggregated row to avoid modifying slices of groups
        aggregated_row_dict = first_row.to_dict() # Start with all columns from the first row
        aggregated_row_dict['alert_reason'] = aggregated_reasons
        aggregated_row_dict['priority_score'] = max_priority
        return pd.Series(aggregated_row_dict)

    valid_group_keys_agg = alerts_df_agg['patient_id'].notna() & alerts_df_agg['encounter_date_obj'].notna()
    if not valid_group_keys_agg.any(): return pd.DataFrame(columns=alerts_df_agg.columns)
    
    final_alerts_df = alerts_df_agg[valid_group_keys_agg].groupby(['patient_id', 'encounter_date_obj']).apply(aggregate_alerts_clinic_final, include_groups=False).reset_index()
    ...
    sort_date_col = 'encounter_date' if 'encounter_date' in final_alerts_df.columns and final_alerts_df['encounter_date'].notna().all() else ('encounter_date_obj' if 'encounter_date_obj' in final_alerts_df.columns else None)
...
def get_district_summary_kpis(enriched_zone_gdf: gpd.GeoDataFrame) -> Dict[str, Any]:
    ...
    if kpis["total_population_district"] > 0 and pd.notna(kpis["total_population_district"]):
        if gdf['avg_risk_score'].notna().any(): kpis["avg_population_risk"] = np.average(gdf['avg_risk_score'], weights=gdf['population']) if gdf['avg_risk_score'].notna().any() else np.nan
        if gdf['facility_coverage_score'].notna().any():kpis["overall_facility_coverage"] = np.average(gdf['facility_coverage_score'], weights=gdf['population']) if gdf['facility_coverage_score'].notna().any() else np.nan
        if gdf['avg_daily_steps_zone'].notna().any(): kpis["population_weighted_avg_steps"] = np.average(gdf['avg_daily_steps_zone'], weights=gdf['population']) if gdf['avg_daily_steps_zone'].notna().any() else np.nan
        kpis["key_infection_prevalence_district_per_1000"] = (gdf['total_active_key_infections'].sum() / kpis["total_population_district"]) * 1000 if kpis["total_population_district"] > 0 else 0.0
    else:
...
def get_trend_data(df: pd.DataFrame, value_col: str, date_col: str = 'encounter_date', period: str = 'D', agg_func: str = 'mean', filter_col: Optional[str] = None, filter_val: Optional[Any] = None) -> pd.Series:
    ...
    if value_col not in trend_df.columns: return pd.Series(dtype='float64') # value_col may not exist if only NaN values in original
    # For nunique, we don't need to dropna for value_col as nunique handles NaNs. For others, it's important.
...
def get_supply_forecast_data(health_df: pd.DataFrame, forecast_days_out: int = 30, item_filter_list: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    # Use the most recent encounter_date for each item to get its latest stock and consumption rate
    supply_status_df = health_df.loc[health_df.groupby('item')['encounter_date'].idxmax()]
    ...
    for _, row in supply_status_df.iterrows():
        ...
        days_rem_start = (cur_stock / cons_rate) if cons_rate > 0.001 else (np.inf if cur_stock > 0 else 0) # Check for zero division
        
        if cons_rate > 0.001: # Only calculate if there is consumption
        ...
        for i, fc_date in enumerate(forecast_dates):
            ...
            # estimated_stockout_date remains constant for all forecast rows of this item
