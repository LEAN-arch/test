...
def _get_theme_color(index=0, fallback_color="#007bff", color_type="general"):
    ...
    try:
        ...
        if custom_template and hasattr(custom_template, 'layout') and hasattr(custom_template.layout, 'colorway') and custom_template.layout.colorway:
        ...
        if plotly_default_template and hasattr(plotly_default_template, 'layout') and hasattr(plotly_default_template.layout, 'colorway') and plotly_default_template.layout.colorway:
...
def set_custom_plotly_theme():
    ...
    theme_primary_text_color = "#343a40"; theme_grid_color = "#e9ecef"; theme_border_color = "#ced4da"; theme_paper_bg_color = "#f8f9fa" ; theme_plot_bg_color = "#FFFFFF"  
    ...
    custom_theme.layout.paper_bgcolor = theme_paper_bg_color; custom_theme.layout.plot_bgcolor = theme_plot_bg_color
    ...
    custom_theme.layout.xaxis = {**axis_common}; custom_theme.layout.yaxis = {**axis_common}
    ...
    if not MAPBOX_TOKEN_SET and app_config.MAPBOX_STYLE not in ["open-street-map", "carto-positron", "carto-darkmatter", "stamen-terrain", "stamen-toner", "stamen-watercolor"]: default_mapbox_style = "open-street-map" 
    ...
    pio.templates["custom_health_theme"] = custom_theme; pio.templates.default = "plotly+custom_health_theme" 
...
def render_kpi_card(title, value, icon, status="neutral", delta=None, delta_type="neutral", help_text=None, icon_is_html=False):
    ...
    final_status_class = status.lower() if status and status.lower() in valid_statuses else "neutral"; final_delta_type = delta_type.lower() if delta_type and delta_type.lower() in valid_delta_types else "neutral"
    semantic_status_class = "status-" + status.lower() if status and status.lower() in ["good", "bad"] else ""
    delta_str = str(delta) if delta is not None and str(delta).strip() else ""; delta_html = f'<p class="kpi-delta {final_delta_type}">{html.escape(delta_str)}</p>' if delta_str else ''
...
def render_traffic_light(message, status, details=""):
    valid_statuses_tl = {"High", "Moderate", "Low", "Neutral"}; dot_status_class = "status-" + (status.lower() if status and status.lower() in valid_statuses_tl else "neutral")
...
def _create_empty_figure(title, height, message="No data available to display."):
    fig = go.Figure(); fig.update_layout(title_text=f"{title} ({message})", height=height, xaxis={'visible': False}, yaxis={'visible': False}, annotations=[dict(text=message, xref="paper", yref="paper", showarrow=False, font=dict(size=14))]); return fig
...
def plot_layered_choropleth_map(gdf: gpd.GeoDataFrame, value_col: str, title: str, id_col: str = 'zone_id', featureidkey_prefix: str = 'properties', color_continuous_scale: str = "Blues_r", hover_cols: list = None, facility_gdf: gpd.GeoDataFrame = None, facility_size_col: str = None, facility_hover_name: str = None, facility_color: str = None, height: int = None, center_lat: float = None, center_lon: float = None, zoom_level: int = None, mapbox_style: str = None ):
    final_height = height if height is not None else app_config.MAP_PLOT_HEIGHT; error_msg_map = "Map data unavailable or configuration error."
    ...
    if gdf_plot[value_col].isnull().all(): gdf_plot[value_col] = gdf_plot[value_col].fillna(0) # Fill NaNs to prevent error in color scale
    ...
    effective_mapbox_style = mapbox_style # User override first
    if not effective_mapbox_style: # If not overridden, try theme default
        try: effective_mapbox_style = pio.templates.default.layout.mapbox.style
        except: effective_mapbox_style = app_config.MAPBOX_STYLE # Fallback to app_config
    if not MAPBOX_TOKEN_SET and effective_mapbox_style not in ["open-street-map", "carto-positron", "carto-darkmatter", "stamen-terrain", "stamen-toner", "stamen-watercolor"]: 
        logger.warning(f"Mapbox token not set for style '{effective_mapbox_style}', defaulting to 'open-street-map' for map '{title}'.")
...
def plot_bar_chart(df_input, x_col: str, y_col: str, title: str, color_col: str = None, barmode: str = 'group', orientation: str = 'v', y_axis_title: str = None, x_axis_title: str = None, height: int = None, text_auto: bool = True, sort_values_by: str = None, ascending: bool = True, text_format: str = ',.0f', color_discrete_map: dict = None):
    ...
    base_hover_bar = f'<b>{final_x_title}</b>: %{{x}}<br><b>{final_y_title}</b>: %{{y:{text_format}}}'; hover_template_bar = base_hover_bar + (f'<br><b>{legend_title}</b>: %{{customdata[0]}}<extra></extra>' if color_col and color_col in df.columns and not df[[color_col]].empty else '<extra></extra>')
...
def plot_donut_chart(data_df_input, labels_col: str, values_col: str, title: str, height: int = None, color_discrete_map: dict = None, pull_segments: float = 0.03, center_text: str = None):
    ...
    default_colorway = pio.templates.default.layout.colorway if hasattr(pio.templates.default, 'layout') and hasattr(pio.templates.default.layout, 'colorway') and pio.templates.default.layout.colorway else px.colors.qualitative.Plotly
    if color_discrete_map: plot_colors_donut_list = [color_discrete_map.get(lbl, default_colorway[i % len(default_colorway)]) for i, lbl in enumerate(df[labels_col])]
    elif hasattr(app_config,"DISEASE_COLORS") and app_config.DISEASE_COLORS: plot_colors_donut_list = [app_config.DISEASE_COLORS.get(lbl, default_colorway[i % len(default_colorway)]) for i,lbl in enumerate(df[labels_col])]
    else: plot_colors_donut_list = [default_colorway[i % len(default_colorway)] for i in range(len(df[labels_col]))]
    fig = go.Figure(data=[go.Pie(labels=df[labels_col], values=df[values_col], hole=0.52, pull=[pull_segments if i < 3 else 0 for i in range(len(df))], textinfo='label+percent', hoverinfo='label+value+percent', insidetextorientation='radial', marker=dict(colors=plot_colors_donut_list, line=dict(color='#ffffff', width=2.2)), sort=True)]) 
    annotations_list_donut = [dict(text=center_text, x=0.5, y=0.5, font_size=18, showarrow=False, font_color=_get_theme_color(0,fallback_color="#343a40"))] if center_text else None
    fig.update_layout(title_text=title, height=final_height, showlegend=True, legend=dict(orientation="v", yanchor="middle", y=0.5, xanchor="right", x=1.18, traceorder="normal"), annotations=annotations_list_donut) 
    return fig
...
def plot_heatmap(matrix_df_input, title: str, height: int = None, colorscale: str = "RdBu_r", zmid: float = 0, text_auto: bool = True, text_format: str = ".2f", show_colorbar: bool = True):
    ...
    zmid_final = zmid if not df_plot_heatmap.empty and pd.Series(z_vals.flatten()).min() < 0 and pd.Series(z_vals.flatten()).max() > 0 else None 
