...
def set_custom_plotly_theme():
    ...
    custom_theme.layout.title = dict(font=dict(size=16, weight='bold'), x=0.02, xanchor='left', y=0.97, yanchor='top', pad=dict(t=25, b=15))
...
def plot_layered_choropleth_map(gdf: gpd.GeoDataFrame, value_col: str, title: str, id_col: str = 'zone_id', featureidkey_prefix: str = 'properties', color_continuous_scale: str = "Blues_r", hover_cols: Optional[List[str]] = None, facility_gdf: Optional[gpd.GeoDataFrame] = None, facility_size_col: Optional[str] = None, facility_hover_name: Optional[str] = None, facility_color: Optional[str] = None, height: Optional[int] = None, center_lat: Optional[float] = None, center_lon: Optional[float] = None, zoom_level: Optional[int] = None, mapbox_style: Optional[str] = None ) -> go.Figure:
    ...
    gdf_for_geojson = gdf_plot[gdf_plot.geometry.is_valid & ~gdf_plot.geometry.is_empty].copy() # GeoJSON needs valid geoms
    ...
    if not MAPBOX_TOKEN_SET and effective_mapbox_style not in ["open-street-map", "carto-positron", "carto-darkmatter", "stamen-terrain", "stamen-toner", "stamen-watercolor"]: effective_mapbox_style = "open-street-map"
...
def plot_annotated_line_chart(data_series: pd.Series, title: str, y_axis_title: str = "Value", color: Optional[str] = None, target_line: Optional[float] = None, target_label: Optional[str] = None, show_ci: bool = False, lower_bound_series: Optional[pd.Series] = None, upper_bound_series: Optional[pd.Series] = None, height: Optional[int] = None, show_anomalies: bool = True, date_format: str = "%b %Y", y_is_count: bool = False) -> go.Figure:
    ...
    if y_is_count:
        ...
        if pd.notna(max_val) and max_val > 0:
            if max_val <= 1 and data_series_numeric.min() >= 0 : yaxis_config['dtick'] = 0.5 # Handles binary 0/1 well
            elif max_val <= 10: yaxis_config['dtick'] = 1
            elif max_val <= 50: yaxis_config['dtick'] = 5
            else: yaxis_config['nticks'] = min(10, int(max_val / 5) +1 if max_val / 5 > 1 else 5)
...
def plot_bar_chart(df_input: pd.DataFrame, x_col: str, y_col: str, title: str, color_col: Optional[str] = None, barmode: str = 'group', orientation: str = 'v', y_axis_title: Optional[str] = None, x_axis_title: Optional[str] = None, height: Optional[int] = None, text_auto: bool = True, sort_values_by: Optional[str] = None, ascending: bool = True, text_format: Optional[str] = None, y_is_count: bool = False, color_discrete_map: Optional[Dict] = None) -> go.Figure:
    ...
    if y_is_count: df[y_col] = df[y_col].round().astype('Int64') # Nullable Int
    ...
    if sort_values_by and sort_values_by in df.columns:
        try: df.sort_values(by=sort_values_by, ascending=ascending, inplace=True, na_position='last', key=(lambda c_data: pd.to_numeric(c_data, errors='ignore') if pd.api.types.is_numeric_dtype(df[sort_values_by]) else c_data.astype(str)))
    ...
    if color_col and color_col in df.columns and color_discrete_map is None:
        if any(keyword in color_col.lower() for keyword in ['condition', 'disease', 'test_type', 'status']):
             unique_color_vals = df[color_col].unique()
             final_color_map = {val: app_config.DISEASE_COLORS.get(str(val), _get_theme_color(i)) for i, val in enumerate(unique_color_vals) if str(val) in app_config.DISEASE_COLORS}
             if not final_color_map : final_color_map = None # Fallback if no matches
    ...
    hover_y_fmt = 'd' if y_is_count else final_text_format_str
    hover_x_fmt = 'd' if (orientation == 'h' and y_is_count) else final_text_format_str # If x is count value
    ...
    texttemplate = (f'%{{y:{final_text_format_str}}}' if text_auto and orientation == 'v' else (f'%{{x:{final_text_format_str}}}' if text_auto and orientation == 'h' else None))
    ...
    if orientation == 'v': main_axis_cfg = {'title_text': final_y_title}; cross_axis_cfg = {'title_text': final_x_title, 'categoryorder':'array', 'categoryarray': df[x_col].tolist() if sort_values_by == x_col else None}
    else: main_axis_cfg = {'title_text': final_x_title}; cross_axis_cfg = {'title_text': final_y_title, 'categoryorder':'total ascending' if ascending and (not sort_values_by or sort_values_by == y_col) else ('total descending' if not ascending and (not sort_values_by or sort_values_by == y_col) else None) }

    if y_is_count:
        main_axis_cfg['tickformat'] = 'd'; main_axis_cfg['rangemode'] = 'tozero'
        max_v = main_val_data.max();
        if pd.notna(max_v) and max_v > 0:
            if max_v <=1 and main_val_data.min()>=0: main_axis_cfg['dtick'] = 0.5
            elif max_v <= 10: main_axis_cfg['dtick'] = 1
            elif max_v <= 50: main_axis_cfg['dtick'] = 5
            else: main_axis_cfg['nticks'] = min(10, int(max_v/10)+1 if max_v/10 > 1 else 5)

    if orientation == 'v': fig.update_layout(yaxis=main_axis_cfg, xaxis=cross_axis_cfg)
    else: fig.update_layout(xaxis=main_axis_cfg, yaxis=cross_axis_cfg)
...
def plot_donut_chart(data_df_input: pd.DataFrame, labels_col: str, values_col: str, title: str, height: Optional[int] = None, color_discrete_map: Optional[Dict] = None, pull_segments: float = 0.03, center_text: Optional[str] = None, values_are_counts: bool = True) -> go.Figure:
    ...
    fig = go.Figure(data=[go.Pie(labels=df[labels_col], values=df[values_col], hole=0.50, pull=[pull_segments if i < 3 else 0 for i in range(len(df))], textinfo='label+percent', hoverinfo='label+value+percent', hovertemplate=f'<b>%{{label}}</b><br>Value: %{{value:{hover_val_fmt}}}<br>Percent: %{{percent}}<extra></extra>', insidetextorientation='radial', marker=dict(colors=plot_colors_final, line=dict(color='#ffffff', width=2)), sort=False)]) 
